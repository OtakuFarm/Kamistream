...
import { Search, Bell, Menu, Shuffle, Palette } from 'lucide-react';
...
export function Topbar({ onMenuClick }: { onMenuClick: () => void }) {
  ...
  return (
    <div className="h-[60px] bg-[var(--bg2)]/70 backdrop-blur-xl border-b border-[var(--border)] flex items-center px-3 sm:px-4 md:px-6 gap-2 sm:gap-3 md:gap-4 shrink-0 sticky top-0 z-50 kami-safe-top">
      ...
      {/* Hamburger */}
      <button className="text-[var(--text3)] hover:text-white p-2 -ml-2 transition-colors shrink-0" onClick={onMenuClick} aria-label="Toggle menu">
      ...
      {/* Search */}
      <div className="flex-1 min-w-0 max-w-[320px] relative ml-auto md:ml-0">
          ...
          <div className="absolute top-full left-0 right-0 mt-2 bg-[var(--card)] border border-[var(--border)] rounded-xl shadow-2xl overflow-hidden z-50 max-h-[400px] overflow-y-auto">
            {isLoading ? (
              ...
              <div className="flex flex-col">
                ...
                <button onClick={() => { if (search.trim()) { setLocation(`/browse?q=${encodeURIComponent(search.trim())}`); setShowDropdown(false); } }}
      ...
      <div className="flex items-center gap-1.5 sm:gap-2 ml-auto">
        ...
        <div className="relative" ref={themeRef}>
          <button onClick={() => setShowTheme(s => !s)} title="Change theme"
            className={`w-9 h-9 flex items-center justify-center rounded-xl bg-[var(--bg3)] border transition-all ${showTheme ? 'border-[var(--purple)] text-[var(--purple)]' : 'border-[var(--border)] text-[var(--text3)] hover:text-white hover:border-[var(--purple)]'}`}>
            ...
            <div className="absolute right-0 top-full mt-2 bg-[var(--card)] border border-[var(--border)] rounded-xl shadow-2xl overflow-hidden z-50 min-w-[140px]">
              ...
              {themes.map(t => (
                <button key={t.id} onClick={(e) => switchTheme(t.id, t.accent, e)}
                  ...
                  {t.name}
                  {theme === t.id && <span className="ml-auto text-[var(--pink)]">✓</span>}
                </button>
              ))}
            </div>
          )}
        ...
        <button title="Notifications — coming soon"
          className="w-9 h-9 flex items-center justify-center rounded-xl bg-[var(--bg3)] border border-[var(--border)] text-[var(--text3)] hover:text-white transition-colors relative cursor-not-allowed opacity-60" disabled>
        ...
        {user ? (
          ...
          <Link href="/login" className="bg-gradient-to-r from-[var(--pink)] to-[var(--purple)] text-white px-3 sm:px-4 py-2 rounded-xl text-[12px] sm:text-[13px] font-bold hover:brightness-110 transition-all">
            Log In
          </Link>
        )}
