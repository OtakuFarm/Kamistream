...
// 2. the generated static sitemap contains /latest-episodes
// NOTE: this used to import api/sitemap-pages.js and read public/sitemap.xml.
// Both are gone — all XML is now generated by scripts/sitemaps.mjs from the
// manifest in src/lib/routes.js. The check reads the manifest directly so it
// runs without needing a completed build.
const { buildRoutes } = await import(new URL('../src/lib/routes.js', import.meta.url));
const manifestPaths = buildRoutes({ genreIds: [], years: [] }).map(r => r.path);
check('manifest: lists /latest-episodes', manifestPaths.includes('/latest-episodes'));

// 3. the manifest marks it as NOT prerendered, so the static build cannot
//    shadow the /latest-episodes -> /api/latest rewrite
{
  const entry = buildRoutes({ genreIds: [], years: [] })
    .find(r => r.path === '/latest-episodes');
  check('manifest: /latest-episodes is not prerendered', entry && entry.prerender === false);
}
...
// The prerenderer builds STATIC_PAGES from the manifest now, so
// /latest-episodes is excluded there because the manifest sets
// prerender:false — not because a literal was deleted from the file.
check('prerender: STATIC_PAGES comes from the manifest', /STATIC_PAGES = buildRoutes/.test(pre));
check('prerender: excludes the year hub (own doc builder)', /r\.path !== '\/best-anime'/.test(pre));
...
// The footer nav is now grouped into labelled columns and the Discover
// column is the one that carries /latest-episodes, so the check looks for
// the href itself rather than for the old flat-array shape.
